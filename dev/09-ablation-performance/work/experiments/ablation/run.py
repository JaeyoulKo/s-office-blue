...
def main() -> None:
    ...
    timeout_seconds = int(config.get("condition_timeout_seconds", 180))
    conditions = (("baseline", None), ("treatment", config["treatment_skill"]))

    def execute_condition(condition: str, skill: str | None, started_at: str) -> dict[str, object]:
        try:
            result = run_codex(
                prompt=prompt,
                payload=payload,
                skill=skill if not treatment_path or condition == "baseline" else None,
                skill_path=(ROOT / treatment_path) if treatment_path and condition == "treatment" else None,
                model=config["model"],
                reasoning_effort=config["reasoning_effort"],
    ...
    with ThreadPoolExecutor(max_workers=2, thread_name_prefix="ablation") as executor:
        futures = {}
        for condition, skill in conditions:
            started_at = utc_now()
            emit_event(condition, "running", timeout_seconds=timeout_seconds)
            future = executor.submit(execute_condition, condition, skill, started_at)
            futures[future] = condition
        for future in as_completed(futures):
            condition = futures[future]
            condition_result = future.result()
            manifest["conditions"][condition] = condition_result
            emit_event(
                condition,
                "failed" if condition_result["error"] else "completed",
