...
description: 사용자가 제공한 이메일·스레드·첨부파일 메타데이터를 전체 문맥으로 구매 승인 요청, 계약·구매·지출 관련 논의, 일반 업무 이메일 중 하나로 분류하고 근거, 요약, 누락 정보, 권장 다음 단계를 구조화한다. 메일 브리핑 또는 분류 Skill A/B 실험에 사용한다. Gmail 조회·발송·라벨 변경·보관·삭제에는 사용하지 않는다.
---
